...
class IntermediateWorkoutResourceIT {
    ...
    @Test
    void getAllIntermediateWorkoutsAsStream() {
        // Initialize the database
        intermediateWorkoutRepository.save(intermediateWorkout).block();

        List<IntermediateWorkout> intermediateWorkoutList = webTestClient
            .get()
            .uri(ENTITY_API_URL)
            ...
            .returnResult(IntermediateWorkoutDTO.class)
            .getResponseBody()
            .map(intermediateWorkoutMapper::toEntity)
            .filter(intermediateWorkout::equals)
            .collectList()
            .block(Duration.ofSeconds(5));

        assertThat(intermediateWorkoutList).isNotNull();
        assertThat(intermediateWorkoutList).hasSize(1);
        IntermediateWorkout testIntermediateWorkout = intermediateWorkoutList.get(0);
        assertThat(testIntermediateWorkout.getDescription()).isEqualTo(DEFAULT_DESCRIPTION);
    }
