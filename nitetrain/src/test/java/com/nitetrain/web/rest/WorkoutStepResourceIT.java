...
class WorkoutStepResourceIT {
    ...
    @Test
    void getAllWorkoutStepsAsStream() {
        // Initialize the database
        workoutStepRepository.save(workoutStep).block();

        List<WorkoutStep> workoutStepList = webTestClient
            .get()
            .uri(ENTITY_API_URL)
            ...
            .returnResult(WorkoutStepDTO.class)
            .getResponseBody()
            .map(workoutStepMapper::toEntity)
            .filter(workoutStep::equals)
            .collectList()
            .block(Duration.ofSeconds(5));

        assertThat(workoutStepList).isNotNull();
        assertThat(workoutStepList).hasSize(1);
        WorkoutStep testWorkoutStep = workoutStepList.get(0);
        assertThat(testWorkoutStep.getTitle()).isEqualTo(DEFAULT_TITLE);
        assertThat(testWorkoutStep.getDescription()).isEqualTo(DEFAULT_DESCRIPTION);
        assertThat(testWorkoutStep.getStepNumber()).isEqualTo(DEFAULT_STEP_NUMBER);
    }
